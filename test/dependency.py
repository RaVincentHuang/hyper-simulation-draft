...
print(f"\nHypergraph created: {len(hypergraph.vertices)} vertices, {len(hypergraph.hyperedges)} hyperedges")
print(f"\nVertices:")
for vertex in hypergraph.vertices:
    poses_str = ", ".join([p.name for p in vertex.poses]) if vertex.poses else "None"
    ents_str = ", ".join([e.name for e in vertex.ents]) if vertex.ents else "None"
    print(f"  Vertex {vertex.id}: '{vertex.text()}' (POS: {poses_str}, ENT: {ents_str})")
print(f"\nHyperedges:")
for i, hyperedge in enumerate(hypergraph.hyperedges):
    vertex_ids = [v.id for v in hyperedge.vertices]
    vertex_texts = [v.text() for v in hyperedge.vertices]
    print(f"  Hyperedge {i}: root={hyperedge.root.id}['{hyperedge.root.text()}'], vertices={vertex_ids}{vertex_texts}, desc='{hyperedge.desc}', range=[{hyperedge.start}, {hyperedge.end}]")
